...
public class MainForm extends Application {
	...
	class StartEventHandler implements EventHandler<MouseEvent>{
		...
		public StartEventHandler(StarService s) {
			this.s = s;
		}
		...
		public void handle(MouseEvent event) {
			...
			StarList s1 =  s.tobeClearedStars(base,mCurretStars);
			if(s1 == null){
				return;
			}
			...
			for(int i = 0;i<s1.size();i++) {
				...
				//从界面容器中找到相应的泡泡糖控件对象
				Label frame = StarFormUtils.findFrame(star1, mStarForm);
				
				
				//执行一组JavaFX提供的动画代码，以渐入渐出的动画方式清除界面上【待消除的泡泡糖】
				StarAnimation.clearStarLable(mStarForm, frame);
				s1.setNull(star1.getPosition().getRow(), star1.getPosition().getColumn());
			}
	...
	private void initGameStars(AnchorPane root) {
		/*class StartEventHandler implements EventHandler<MouseEvent>{
			...
			public void handle(MouseEvent event){
				Label starFrame = (Label) event.getTarget(); //获取被点击的泡泡糖控件
				Star base = StarFormUtils.convert(starFrame); // 将视图转换为泡泡糖对象
			}
			//System.out.println("");
		}*/
		...
		for (int i = 0; i < mCurretStars.size(); i++) {
			...
			/******************** PRJ-BU2-JAVA-007 Task1 ********************/
				...
				StartEventHandler s = new  StartEventHandler(starService);
				starFrame.setOnMouseClicked(s);
