...
public class StarServiceImpl implements StarService {
	...
	private ArrayList<Integer> getNullColumn(StarList currStarList) {
		ArrayList<Integer> starList = new ArrayList<Integer>();
		Star star = new Star();
		for(int i = 0;i< MAX_COLUMN_SIZE;i++){
			star = currStarList.lookup(MAX_COLUMN_SIZE -1,i);
			if(star == null){
				starList.add(i);
			}
		}
	...
	public StarList getXMovedStars(StarList currentStarList) {
		...
		ArrayList<Integer> nullStarList = getNullColumn(currentStarList);
		StarList movedXList = new StarList();
		if(nullStarList == null){
			return null;
		}
		int xDis = 0;
		MovedStar movedStar = new MovedStar();
		//System.out.println(nullStarList);
		for(int i = nullStarList.get(0);i< MAX_COLUMN_SIZE;i++){
			if(nullStarList.contains(i)){
				xDis ++; //是空列
			}
			else{
				Star star = new Star();
				for(int j = MAX_ROW_SIZE -1;j >=0;j--){
					star = currentStarList.lookup(j,i);
					if(star == null){
						break;
					}
					else{
						movedStar = StarsUtil.toMovedStar(star);
						movedStar.setMovedPosition(new Position(movedStar.getPosition().getRow(),movedStar.getPosition().getColumn() - xDis));
						movedXList.add(movedStar);
					}
				}
			}
		}
